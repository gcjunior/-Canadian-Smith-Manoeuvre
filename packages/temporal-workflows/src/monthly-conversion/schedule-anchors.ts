/**
 * Workflow-safe copy of domain schedule anchors (no Node-only imports).
 * Keep in sync with `@csm/domain` monthly-payment-schedule policy.
 */
...
export function deriveAnchorsFromCheckInstant(input: {
  ...
  return {
    expectedPaymentDate: `${payment.year}-${pad2(payment.monthIndex0 + 1)}-${pad2(clampedDay)}`,
    paymentPeriod: `${payment.year}-${pad2(payment.monthIndex0 + 1)}`,
